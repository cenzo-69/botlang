...
module.exports = async (context, args) => {
  ...
  if (!guildID || !query) return '[error: $findRole — guildID and query are required]';
  try {
    const guild = await context.client?.guilds.fetch(guildID);
    if (!guild) return '[error: $findRole — guild not found]';
    const q    = query.toLowerCase();
    const role = guild.roles.cache.find(r => r.id === query || r.name.toLowerCase().includes(q));
    return role?.id ?? '';
  } catch (err) { return `[error: $findRole — ${err.message}]`; }
};
