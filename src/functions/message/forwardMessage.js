...
module.exports = async (context, args) => {
  ...
  if (!channelID || !messageID) return '[error: Could not resolve source channel/message!]';
  try {
    const srcCh = await context.client?.channels.fetch(channelID);
    const msg   = await srcCh?.messages.fetch(messageID);
    if (!msg)   return '[error: Source message not found!]';
    const dstCh = await context.client?.channels.fetch(targetID);
    if (!dstCh) return '[error: Target channel not found!]';
