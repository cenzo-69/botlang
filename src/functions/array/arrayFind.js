...
module.exports = {
  ...
  execute: async (context, args) => {
    ...
    if (!name) return '[error: $arrayFind — variable name is required]';
    const raw = context.variables.get(`__array_${name}__`);
    if (!raw) return `[error: $arrayFind — array "${name}" does not exist]`;
    let arr;
    try { arr = JSON.parse(raw); } catch { return '[error: $arrayFind — corrupted array data]'; }
