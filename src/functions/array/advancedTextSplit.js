...
module.exports = async (context, args) => {
  ...
  if (pairs.length < 2) return '[error: $advancedTextSplit — requires text + at least one split;index pair]';
  for (let i = 0; i < pairs.length - 1; i += 2) {
    const sep   = String(pairs[i]);
    const index = parseInt(pairs[i + 1]);
    if (isNaN(index)) return `[error: $advancedTextSplit — index at position ${i + 1} is not a number]`;
