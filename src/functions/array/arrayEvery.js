'use strict';
// $arrayEvery[variable;itemVar;condition]  — returns "true" if all elements pass condition
module.exports = {
  lazy: [1, 2],
  execute: async (context, args) => {
    ...
    if (!name) return '[error: $arrayEvery — variable name is required]';
    const raw = context.variables.get(`__array_${name}__`);
    if (!raw) return `[error: $arrayEvery — array "${name}" does not exist]`;
    let arr;
    try { arr = JSON.parse(raw); } catch { return '[error: $arrayEvery — corrupted array data]'; }
    ...
    for (const item of arr) {
      ...
      if (!Array.isArray(condNodes)) return 'false';
