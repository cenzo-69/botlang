'use strict';
// $arrayFindIndex[variable;itemVar;condition]  — returns 0-based index of first match
module.exports = {
  lazy: [1, 2],
  execute: async (context, args) => {
    const name      = String(args[0] !== undefined ? args[0] : '').trim();
    const varNodes  = args[1];
    const condNodes = args[2];
    if (!name) return '[error: $arrayFindIndex — variable name is required]';
    const raw = context.variables.get(`__array_${name}__`);
    if (!raw) return `[error: $arrayFindIndex — array "${name}" does not exist]`;
    let arr;
    try { arr = JSON.parse(raw); } catch { return '[error: $arrayFindIndex — corrupted array data]'; }
    const itemVar = Array.isArray(varNodes) ? await context.child().executeNodes(varNodes) : String(varNodes);
    for (let i = 0; i < arr.length; i++) {
      const child = context.child();
      child.variables.set(itemVar, String(arr[i] ?? ''));
      if (!Array.isArray(condNodes)) continue;
      const result = await child.executeNodes(condNodes);
      if (result === 'true' || result === '1') return String(i);
    }
    return '-1';
  },
};
