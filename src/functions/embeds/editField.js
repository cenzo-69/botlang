...
module.exports = async (context, args) => {
  ...
  if (isNaN(fieldIdx)) return '[error: $editField — fieldIndex must be a valid integer]';
  const embeds = context.variables.get('__embeds__');
  if (!embeds) return '[error: $editField — no embeds are being built]';
  try {
    const arr   = JSON.parse(embeds);
    const embed = arr[embedIdx];
    if (!embed?.fields?.[fieldIdx]) return '[error: $editField — field not found at given index]';
    ...
    return '';
  } catch (err) { return `[error: $editField — ${err.message}]`; }
};
