...
module.exports = async (context, args) => {
  ...
  if (isNaN(fieldIdx)) return '[error: $deleteField — fieldIndex must be a valid integer]';
  const embeds = context.variables.get('__embeds__');
  if (!embeds) return '[error: $deleteField — no embeds are being built]';
  try {
    const arr = JSON.parse(embeds);
    const embed = arr[embedIdx];
    if (!embed) return '[error: $deleteField — embed at specified index does not exist]';
    if (!embed.fields) return '';
    embed.fields.splice(fieldIdx, 1);
    context.variables.set('__embeds__', JSON.stringify(arr));
    return '';
  } catch (err) { return `[error: $deleteField — ${err.message}]`; }
};
